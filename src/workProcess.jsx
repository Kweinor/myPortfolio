function WorkProcess({darkMode}) {
  ...
  return (
    <div className={`py-0 ${ darkMode?  "bg-[#0B1226]": "bg-[#f8f5ee]   text-gray-800"}`}>
      ...
      <h2 className={`text-4xl font-bold text-center ${darkMode? "text-[#503f82]": "text-[#272b53]"} mb-12`}>My Work Process</h2>
...
From understanding your goals to designing, building and deploying your product, every step is handled with clarity and purpose.</p>
        ...
        <div className="flex flex-col lg:grid md:grid-cols-2 gap-6 md:gap-20 lg:gap-10 max-w-6xl mx-auto px-5">
        {steps.map((item, index) => (
          <div
            key={index}
            className={`${darkMode? "bg-[#050014]/70":"bg-[#f8f5ee]"} p-6 rounded-2xl shadow-xl w-full md:w-80 hover:scale-105 transition duration-300 ${index % 2 === 1 ? "lg:translate-y-10" : ""}`}        >
